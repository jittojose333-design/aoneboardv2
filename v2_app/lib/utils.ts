...
export type DateRange = 'today' | 'this-week' | 'this-month' | 'last-3-months' | 'this-year' | 'ytd' | 'custom' | 'all-time';

const parseDate = (dateStr: string): Date => {
    if (!dateStr) return new Date();
    // Handle DD/MM/YYYY format (Common in India/NREGA)
    if (dateStr.includes('/')) {
...
export const isDateInRange = (dateStr: string, range: DateRange, customStart?: string, customEnd?: string): boolean => {
    ...
    if (isNaN(date.getTime())) return true; // Fail safe: Include invalid dates rather than hiding them

    const startOfToday = new Date(now);

    switch (range) {
        case 'today': return date.getTime() === startOfToday.getTime();
        case 'this-week': {
        ...
        case 'last-3-months': {
            ...
            threeMonthsAgo.setMonth(now.getMonth() - 2); // Current + 2 prev = 3 months window
